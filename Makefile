...
IMAGE_TAG := treasure-erp-orders:${GIT_VERSION_HASH}
...
deploy: push check-env
	gcloud beta run deploy treasure-erp-orders \
		--project ${PROJECT_ID} \
		--image ${REMOTE_IMAGE_TAG} \
		--allow-unauthenticated \
		--platform managed \
		--region us-east1 \
		--set-env-vars=NODE_ENV=${NODE_ENV},DB_HOST=${DB_HOST},DB_USERNAME=${DB_USERNAME},DB_PASSWORD=${DB_PASSWORD},DB_DATABASE=${DB_DATABASE},PRODUCTS_API_URL=${PRODUCTS_API_URL} \
		--memory=1Gi
